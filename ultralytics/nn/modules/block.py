...
class GELAN(nn.Module):
    def __init__(self, c1, c2, n=1, shortcut=True, g=1, e=0.5):
        """
        GELAN模块（适配YOLOv8的C2F替换）
        Args:
            c1: 输入通道数
            c2: 输出通道数
            n: 模块重复次数（对应C2F的n）
            shortcut: 是否使用残差连接
            g: 分组卷积的组数（默认1，避免类型错误）
            e: 扩展比例（控制中间通道数）
        """
        super().__init__()
        self.cv1 = nn.Conv2d(c1, c2, 1, 1, bias=False)  # 1x1卷积降维
        self.cv2 = nn.Conv2d(c1, c2, 1, 1, bias=False)  # 1x1卷积降维
        self.cv3 = nn.Conv2d(c2, c2, 3, 1, 1, groups=g, bias=False)  # 分组卷积
        self.cv4 = nn.Conv2d(c2, c2, 1, 1, bias=False)  # 逐点卷积
        self.bn1 = nn.BatchNorm2d(c2)
        self.bn2 = nn.BatchNorm2d(c2)
        self.bn3 = nn.BatchNorm2d(c2)
        self.act = nn.GELU()
        self.shortcut = shortcut and c1 == c2  # 残差连接条件：输入输出通道一致
        self.n = n  # 重复次数

    def forward(self, x):
        y1 = self.act(self.bn1(self.cv1(x)))  # 主分支1
        y2 = self.cv2(x)  # 主分支2
        for _ in range(self.n):
            y2 = self.act(self.bn2(self.cv3(y2)))  # 分组卷积
            y2 = self.act(self.bn3(self.cv4(y2)))  # 逐点卷积
        return y1 + y2 if self.shortcut else y1  # 残差连接
