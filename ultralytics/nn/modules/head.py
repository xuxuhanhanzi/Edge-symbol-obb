...
class OBB(Detect):
    ...
    YOLOv8 OBB detection head with ENHANCED Angle Branch for INT8 Quantization.

    彻底重写全部分支，强制 NPU 瓶颈压缩与 reg_max=8 对齐。
    ...
    def __init__(self, nc=80, ne=1, ch=()):
        ...
        """Initialize OBB with number of classes and channels."""
        ...
        # 1. 极限压缩 DFL 通道，拯救 NPU 内存带宽

        self.reg_max = 8  

        # 必须同步更新 self.no (每个 Anchor 的输出总通道数：15类 + 8*4框 = 47)
        ...
        # 2. 强行将所有分支的中间卷积通道数死锁在 64，极致压缩！

        c2 = 64                          # 边框中间通道

        c3 = max(64, min(self.nc, 100))  # 分类中间通道

        c4 = 64                          # 角度中间通道

        

        # 3. 覆写父类的 cv2 (边框分支)：强制输出 4 * reg_max (即 32) 个通道
        ...
        # 4. 覆写父类的 cv3 (分类分支)：防止其膨胀，强制输出 nc (即 15) 个通道
        ...
        # 5. 增强型角度分支 (Enhanced Angle Branch)
    ...
    #     # 依赖父类 (Detect) 的前向传播获取常规的 box 和 cls 特征
    ...
    #     # ONNX 四头导出模式
    #     if self.export and self.format == "onnx_4head":
    #         outs = []

    #         for i in range(self.nl):
    #             box = self.cv2[i](x[i])
    #             cls = torch.sigmoid(self.cv3[i](x[i]))
    #             obj = torch.clamp(cls.sum(1, keepdim=True), 0, 1)
    ...
    #         return outs[0], outs[1], outs[2], angle_out
    ...
    def forward(self, x):
        ...
        # ============================================================
        # ONNX 四头导出模式
        # 注意：必须在 super().forward(x) 之前执行
        # 因为此时 x 还是 [P3, P4, P5] 三个特征图
        # ============================================================
        if self.export and self.format == "onnx_4head":
            outs = []
            ...
            for i in range(self.nl):
                ...
                cls = torch.sigmoid(self.cv3[i](x[i]))  # [B, nc, H, W]

                # 可选 objectness，YOLOv8 原本没有 obj，这里用 cls_sum 近似给 RKNN 后处理使用
                obj = torch.clamp(cls.sum(1, keepdim=True), 0, 1)
