...
# ================= 配置参数 =================
ORIG_IMG_DIR = "/home/lab/桌面/ultralytics_yolov8-main/VOCdevkit/VOC2007/OBB/images"  # OBB原图目录
ORIG_LABEL_DIR = "/home/lab/桌面/ultralytics_yolov8-main/VOCdevkit/VOC2007/OBB/labels"  # OBB原标注目录
AUG_IMG_DIR = "/home/lab/桌面/ultralytics_yolov8-main/VOCdevkit/VOC2007/OBB/augmented/images"  # 增强图保存目录
AUG_LABEL_DIR = "/home/lab/桌面/ultralytics_yolov8-main/VOCdevkit/VOC2007/OBB/augmented/labels"  # 增强OBB标注保存目录
...
if __name__ == "__main__":
    ...
    for i, img_name in enumerate(img_files):
        ...
        try:
            # 复制原始图片
            shutil.copy2(img_path, os.path.join(AUG_IMG_DIR, img_name))
            # 复制原始标注
            shutil.copy2(label_path, os.path.join(AUG_LABEL_DIR, Path(img_name).stem + ".txt"))
            total_original_copied += 1
        except Exception as e:
            print(f"[警告] 复制原始文件失败: {img_name} | {e}")
            continue
    ...
    print(f"成功复制原始文件: {total_original_copied}")
    print(f"生成增强图: {total_aug_imgs}")
    print(f"跳过文件: {skipped_files}")
    print(f"总文件数（原始+增强）: {total_original_copied + total_aug_imgs}")
